...
#if DEBUG_USBHOST_MSD
        printf("usbhost_msd_read10(%u): %d\n", (unsigned int)lba, ret);
#endif
        if (ret == 0)
        {
            return blockcount;
...
int usbhost_msd_write_blocks( usbhost_msd_t * drv, int sid, uint8_t *data, uint32_t lba, uint16_t blockcount )
{
    ...
    if (drv->usbdevices[sid].inuse)
    {
        err = usbhost_msd_write10(drv->usbdevices[sid].usbdevicedriver, data, lba, blockcount, drv->usbdevices[sid].info.blksize);
#if DEBUG_USBHOST_MSD
        printf("usbhost_msd_write10(%d): %u\n", (unsigned int)lba, err);
#endif
        if (err == 0)
        {
            return blockcount;
