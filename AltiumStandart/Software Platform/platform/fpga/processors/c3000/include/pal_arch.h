...
{
     uint32_t oldmask = __mfc0(TSK3000_COP_InterruptEnable);
#if ( __POSIX_KERNEL__ == 0 )
...
{
    __mtc0( tsk3000_posix_interrupts_savemask , TSK3000_COP_InterruptEnable );
}

inline void   pal_architecture_posix_interrupts_disable ()
{
    uint32_t mask = __mfc0(TSK3000_COP_InterruptEnable);
    __mtc0( mask & ~(posix_interrupts_bitmask) & ~(1<<PAL_ARCH_3000_TIMER_INTERRUPT_NUMBER) , TSK3000_COP_InterruptEnable );
    tsk3000_posix_interrupts_savemask = mask;
}
#endif
