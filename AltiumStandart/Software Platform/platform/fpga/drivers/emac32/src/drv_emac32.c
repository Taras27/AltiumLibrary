...
{
    // test & clear overflow
    if (EMAC32_RX_STATUS(base) & (EMAC32_RX_STATUS_INT_LACKOFSPACE | EMAC32_RX_STATUS_INT_OVERFLOW ))
    {
        __nop();                             // ERIK: WHY IS THIS NEEDED?
        EMAC32_RX_CMD(base) |= EMAC32_RX_CMD_CLR_OVERFL;
        EMAC32_RX_CMD(base) &= ~EMAC32_RX_CMD_CLR_OVERFL;
    }
...
static uint32_t * emac32_transmit_getbuf(emac32_t * restrict drv, uint32_t size)
{
    ...
    do
    {
        interrupts_disable(); // PATCH
        corebuf = emac32_txbuffer(drv->baseaddress, size);
        interrupts_enable();// PATCH
        if (corebuf != NULL)
        {
            *corebuf = size;
            break;
