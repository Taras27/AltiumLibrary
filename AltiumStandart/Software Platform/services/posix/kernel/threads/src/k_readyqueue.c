...
posix_thread_t*         posix_rq_rm_head(int pri)
{
        ...
        /* dequeue head  */
        *_posix_readyqueue[pri]         = thread->queue;

        /* old 'head' is not queued anymore */
        thread->queue           = (posix_thread_t*)0;
...
void    posix_rq_rm_thread(posix_thread_t* thread,int pri)
{
        ...
        while(*ptr!=thread) { ptr = &(*ptr)->queue; }
        
        *ptr            = (posix_thread_t*) thread->queue;
        thread->queue   = (posix_thread_t*) 0;
...
void posix_rq_add_tail(posix_thread_t* thread, int pri)
{
        ...
        {
                /* not empty */
                thread->queue = *_posix_readyqueue[pri];
                *_posix_readyqueue[pri] = thread;
                _posix_readyqueue[pri] = (posix_thread_t**)&thread->queue;
        }
        else
        {
                /* empty */
