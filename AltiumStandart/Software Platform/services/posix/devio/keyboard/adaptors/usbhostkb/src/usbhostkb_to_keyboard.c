...
static int usbhostkb_to_keyboard_getchar(usbhostkb_to_keyboard_t * adapter)
{
    ...
    while ((scancode = usbhost_keyboard_get_scancode(adapter->usbhost_keyboard)) != 0xFFFF)
    {
        ...
        else
        {
            if (key >= 'a' && key <= 'z')
            {
                if ((shift & (USBHOST_KEYBOARD_MOD_LEFTCTRL | USBHOST_KEYBOARD_MOD_RIGHTCTRL)) || (adapter->shift_state & USBHOST_KEYBOARD_LED_CAPSLOCK))
                {
                    key -= ('a' - 'A');
                }
            }
            else if (shift & (USBHOST_KEYBOARD_MOD_LEFTCTRL | USBHOST_KEYBOARD_MOD_RIGHTCTRL))
            {
