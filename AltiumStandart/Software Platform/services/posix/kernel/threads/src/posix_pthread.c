...
bool    posix_qthread_rm ( posix_thread_t * th, posix_threadqueue_t* q )
{
        ...
        if (q->head != NULL)
        {
                ...
                {
                        thread = q->head;
                        for (;thread->queue !=NULL;thread=thread->queue)
                        {
                                if (thread->queue == th)
                                {
                                        thread->queue = th->queue;
                                        if (q->tail == th)
                                        {
                                                q->tail = thread;
                                        }
...
bool    posix_cond_thread_sigmask(pthread_cond_t* cond, sigset_t* mask)
{
        ...
        for (; thread != NULL; thread=thread->queue)
        {
...
#endif
        nthread->queue          = (posix_thread_t*)0;
        nthread->mutex          = (pthread_mutex_t*)0;
