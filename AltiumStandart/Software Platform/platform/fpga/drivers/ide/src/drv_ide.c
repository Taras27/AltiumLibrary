...
// PIO mode 0 delays  TODO: make CLK dependant
#define SETUP_DELAY_SAFE                ((19 << 8) | (7 << 3) | 3)
...
{
    ide_t *ide;
    drv_ide_cfg_instance_t *cfg;
    per_ide_cfg_instance_t *per_cfg;
#if ( DRV_IDE_USE_MUTEX != 0 )
...
extern int ide_open_device( ide_t * restrict ide )
{
    ...
    uint16_t *buf16;
    uint16_t piodelay_setup;
    uint16_t piodelay_pulse;
    uint16_t piodelay_recover;
    ...
    IDE_MUTEX_PREPARE();
    IDE_MUTEX_LOCK( ide );
    IDECORE_SETUP( ide->baseaddr ) = ( ide->slave ? IDECORE_SETUP_CSEL : 0 ) | SETUP_DELAY_SAFE;
...
#if 0
    // set up datatransfer speed for IDE core
    set_register( ide->baseaddr, IDE_FEATURES, FEATURES_SETTRANSFERMODE );
    set_register( ide->baseaddr, IDE_SECTOR_COUNT, 0x00 );      // default PIO mode
    set_register( ide->baseaddr, IDE_COMMAND, IDE_COMMAND_SET_FEATURES );

    // PIO mode 0 at 50 MHz
    piodelay_setup = 3;
    piodelay_pulse = 7;
    piodelay_recover = 19;
#endif

    // PIO mode 2 at 50 MHz
    piodelay_setup = 1;
    piodelay_pulse = 3;
    piodelay_recover = 5;
    ...
    IDECORE_SETUP( ide->baseaddr ) =
        ( ide->
          slave ? IDECORE_SETUP_CSEL : 0 ) | ( piodelay_recover << 8 ) | ( piodelay_pulse << 3 ) |
        piodelay_setup;
    debug_printf( "PIO delays set to setup=%i pulse=%i recover=%i cycles\n", piodelay_setup, piodelay_pulse,
                  piodelay_recover );
...
{
    int pioloop = SETUP_DELAY_SAFE;     // safe fallback to PIO-0 speed

    IDE_MUTEX_PREPARE();
    IDE_MUTEX_LOCK( ide );

    IDECORE_SETUP( ide->baseaddr ) = pioloop | ( ide->slave ? IDECORE_SETUP_CSEL : 0 );
