...
static void connect_cb(usbhost_msd_t *drv, int sid, bool on, void *data)
{
    ...
    if (on)
    {
        ...
        if (adaptor->automount)
        {
            ...
            /*
             * if not RDONLY then mount with SYNC otherwise we might/will lose
             * data when the device is removed before cached data is flushed to
             * the device
             */
            flag = adaptor->readonly ? MOUNT_FLAG_RDONLY : MOUNT_FLAG_SYNC;
...
static bool blockio_usbhost_msd_blockio_sanity_check( void *device, uint32_t blkstart, uint32_t blkcnt, uint32_t blktotal )
{
    ...
    /* just pretent the information is correct */
    return true;
}
