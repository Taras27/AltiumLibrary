...
extern bool interrupt_register_native( uint32_t number, void* argc, interrupt_native_handler_t handler )
{
	if (number >= PAL_ARCH_MAXINTRS) return false;
    pal_architecture_set_native_handler( number, handler );
    interrupt_table[number].context = argc;
	return true;
}
